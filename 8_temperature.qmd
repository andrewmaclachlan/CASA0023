...
* You should select an area that isn't too cloudy or change the image date seach values
...
To calculate the percentile of the mean tempearture per spatial unit, i downloaded my data from GEE and ran it in R....
...
```{r}
library(terra)
temp <- rast("prac_8/subtracted_mean_HI.tif")

tmap_mode("view")

interactive <- tm_shape(temp) +
...
Now we have the tempearture per ward, we could explore factors of the built envrionemnt that might contirbute to that temperature....such as...buildings, lack of vegetation, lack of water areas and so on. 
